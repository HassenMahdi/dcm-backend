...
from app.engine.sinks.document_sink import DocumentSinkEngine
...
def start_upload(flow: FlowContext):
    try:
        ...
        # engine = DocumentSinkEngine(flow)
        engine.upload(df)
...
def stage_adf_upload(flow_id):
    flow = FlowContext().load({"_id":flow_id})
    if not flow.run_id:
        @copy_current_request_context
        def ctx_bridge():
            start_adf_upload(flow)
        thr = threading.Thread(target=ctx_bridge)
        thr.start()
        ...
        return {'ADF upload has Already been started.'}


def start_adf_upload(flow):
