...
def apply_mapping_transformation(df, mapping_id, target_fields):
    ...
    transformed_df = pd.DataFrame(columns=list(target_fields.keys()), index=df.index)
    ...
    for target, source in mappings.items():
        ...
        elif source in df.columns.tolist():
            transformed_df[target] = df[source]
        elif source[0] in df.columns.tolist():
...
def start_check_job(job_id, file_id, worksheet_id, mapping_id, domain_id, is_transformed, user_id, modifications=None):
    ...
    if is_transformed:
        worksheet_id = worksheet_id.replace('\\','/')
    ...
    try:
        ...
        for column in result_df.columns.values:
            ...
            for index in result_df.index:
                count = index
                if index in indexes:
                    line_errors = result.setdefault(count, {})\
                        .setdefault(field_code, {})\
                        .setdefault(error_type, [])
                    ...
                    line_errors.append({"check_type": check_type, "error_message": error_message})
                else:
